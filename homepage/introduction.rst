...
* `Maurice Ravel – Piano Pieces <https://github.com/DCMLab/ravel_piano/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the piano piece *Jeux d’eau* has the following files:

-  ``MS3/Ravel_-_Jeux_dEau.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/Ravel_-_Jeux_dEau.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/Ravel_-_Jeux_dEau.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/Ravel_-_Jeux_dEau.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/Ravel_-_Jeux_dEau.tsv')
   notes = ms3.load_tsv('notes/Ravel_-_Jeux_dEau.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/ravel_piano/issues>`__ and feel free to fork and submit pull requests.
